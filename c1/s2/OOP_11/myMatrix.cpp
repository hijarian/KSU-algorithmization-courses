...
void vecPrint(doublevec* vec, int precision)
{
	doublevec::iterator It=(*vec).begin();
	cout << "{ " << fixed;
	for(It; It!=(*vec).end(); ++It)
	{
		cout << setw(intL) << setprecision(precision) << *It;
		if(It!=(*vec).end()-1)
			cout << ';';
		cout << ' ';
	}
...
intvec* vecFill(intvec* work, int Elems, int rSeed)
{
	...
	{
		(*work).push_back((int)(rand()/0x2FFF-rand()/0x2FFF));
	}
	return work;
}
