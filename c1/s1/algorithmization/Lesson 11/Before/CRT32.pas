...
var
  ...
  function Oem(S: string): string; overload;
  function Oem(S: char): char; overload;
  function toChar(S: string): string; overload;
  function toChar(S: char): char; overload;
...
{      LKey := Char(LInputRec.Event.KeyEvent.wVirtualKeyCode);
      ...
      ToAscii(LInputRec.Event.KeyEvent.wVirtualKeyCode,
        LInputRec.Event.KeyEvent.wVirtualScanCode, KeybState, @LKey, 0);

      LKey := Oem(LKey);
}
{      if (LInputRec.Event.KeyEvent.wVirtualKeyCode=16) and
         ((LInputRec.Event.KeyEvent.dwControlKeyState and LEFT_CTRL_PRESSED)<>0)
         or (LInputRec.Event.KeyEvent.wVirtualKeyCode=17) and
      ...
      if (LInputRec.Event.KeyEvent.wVirtualKeyCode=16) or
        ...
        Lkey := LInputRec.Event.KeyEvent.AsciiChar;
...
begin
  ...
  //две строки ниже отключены И.З.Ахметзяновым
  ...
  //управление раскладками (загрузка раскладок)
  //hRU := LoadKeyboardLayout('00000409', KLF_SUBSTITUTE_OK);
  //hEN := LoadKeyboardLayout('00000419', KLF_SUBSTITUTE_OK);

  //hCur := GetKeyboardLayout(0);
  //hprev := ActivateKeyboardLayout(hrusLayout, KLF_REORDER);
end;
...
begin
  //строка ниже отключена И.З.Ахметзяновым
